...
至于不用Logits计算的原因，是计算Logits要计算整个词汇表，非常浪费资源，要知道输入法对延迟要求极高
...
社区没有人能够提供支持，因此我开始使用rust自己实现，我对API不熟悉，所以代码都是AI，不过我会认真审查，而Rust的安全性也非常适合AI编程，几天就整出核心功能了。但是Rime文档稀缺，我没写过C++项目，被编译失败劝退了好久。

直到到最我才重新构想，使用更加简单的llama.cpp为推理引擎，它的API也比原先的ort好写，于是只用写一个类就实现了原来一大堆代码的功能。但是又给报了难以找到问题的段错误
